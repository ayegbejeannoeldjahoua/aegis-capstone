          ...
          <tbody className="divide-y divide-slate-700/50">
            {(decisions || []).slice(0, 20).map((row, i) => (
              <tr key={`${row.trace_id}-${row.action}-${i}`} className="text-slate-300">
                ...
                <td className="py-2 pr-4 text-slate-400">{row.latency_ms == null ? "No data" : `${Number(row.latency_ms).toFixed(1)} ms`}</td>
                <td className="py-2 text-slate-400 truncate max-w-[260px]">{row.reason || "—"}</td>
              </tr>
            ))}
            {(!decisions || decisions.length === 0) && (
              <tr><td colSpan={8} className="py-4 text-slate-500">No decisions today</td></tr>
            )}
          </tbody>
        </table>
      </div>
